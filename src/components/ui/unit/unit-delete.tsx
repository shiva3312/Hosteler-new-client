//Copyright (c) Shivam Chaurasia - All rights reserved. Confidential and proprietary.
import { Button, Modal, Text, UnstyledButton } from '@mantine/core';
import { IconTrash } from '@tabler/icons-react';
import { useState } from 'react';

import { useNotifications } from '@/components/ui/core/notifications';
import { UnitResponse } from '@/interfaces/unit.interface';
import { useDeleteUnit } from '@/lib/api/unit/delete-unit';

type DeleteUnitProps = {
  unit?: UnitResponse;
};

export const DeleteUnit = ({ unit: targetedUnit }: DeleteUnitProps) => {
  const { addNotification } = useNotifications();
  const [opened, setOpened] = useState(false);

  const deleteUnitMutation = useDeleteUnit({
    mutationConfig: {
      onSuccess: () => {
        addNotification({
          type: 'success',
          title: 'Unit deleted',
        });
        setOpened(false);
      },
      onError: () => {
        addNotification({
          type: 'error',
          title: 'Failed to delete unit',
        });
      },
    },
  });

  return (
    <>
      <Modal
        opened={opened}
        onClose={() => setOpened(false)}
        title="Delete Unit"
        centered
      >
        <Text>{`Are you sure you want to delete ${targetedUnit?.name} unit?`}</Text>
        <Button
          mt="md"
          color="red"
          loading={deleteUnitMutation.isPending}
          onClick={() =>
            targetedUnit?._id &&
            deleteUnitMutation.mutate({
              unitId: targetedUnit?._id,
            })
          }
          fullWidth
        >
          Confirm Delete
        </Button>
      </Modal>

      <UnstyledButton onClick={() => setOpened(true)}>
        <IconTrash size={25} />
      </UnstyledButton>
    </>
  );
};
